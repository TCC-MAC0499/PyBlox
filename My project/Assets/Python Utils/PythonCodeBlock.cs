...
public class PythonCodeBlock : IComparable<PythonCodeBlock>
{
    ...
    public double codeLineBreakTolerance = 0.0;
    ...
    public double SetLineBreakToleranceFromBorder(List<Vector2> border)
    {
        var sideLengthSum = 0.0;
        for (var idx = 0; idx < border.Count; idx++)
        {
            var corner = border[idx];
            var nextCorner = border[(idx + 1) % border.Count];
